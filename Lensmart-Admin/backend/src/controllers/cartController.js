...
export const addToCart = async (req, res) => {
  try {
    let { img, product_id, name, mrp, sell_price, rating, reviews, gender } =
      req.body;
    ...
    const [exist] = await pool.query(
      "SELECT * FROM cart WHERE client_id = ? AND  product_id = ?",
...
export const incQty = async (req, res) => {
  try {
    ...
    // console.log('id ----', id )
    const [result] = await pool.query(
      `UPDATE cart SET quantity = quantity + 1, updated_at = NOW() WHERE product_id = ?`,
      [id]
    );
    ...
    res.status(201).json({ message: "update qty cart successfully" });
  } catch (error) {
    console.error("Error cart:", error);
    res.status(500).json({ error: "Failed to cart item" });
  }
...
export const decQty = async (req, res) => {
  try {
    ...
    // console.log('id ----', id )
    const [result] = await pool.query(
      `UPDATE cart SET quantity = quantity - 1, updated_at = NOW() WHERE product_id = ?  AND quantity > 1`,
      [id]
    );
