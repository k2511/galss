...
const ColourContactLens = () => {
  ...
  const updatedRanges = priceRanges.map((range) => {
    const count = products.filter((lens) =>
      Number(
        lens.category_name === "Colour Contacts"  && 
  ...
  const filteredProducts = products.filter((p) => {
    ...
    if (p.category_name === "Colour Contacts" ) {
    ...
    <div className=" text-center ">
      {!showFilter && (
        <div className="w-full  mx-auto flex flex-row overflow-y-auto justify-between sm:gap-2 gap-1 items-start ">
          <aside className="  sticky lg:block hidden top-0 lg:h-[85vh] lg:w-1/5 min-w-1/4 border
           rounded-xl shadow-sm bg-white sm:p-4 p-0  overflow-y-auto
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:gap-6 gap-3 ">
              {filteredProducts.length > 0 ? (
                filteredProducts.slice(0,showItem).map((item) => {
                  ...
                  let matchedReview =
                    item.category_name === "Colour Contacts"  && 
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        ...
                        window.open(`/colour-contact-lenses/${encodeURIComponent(item.name)}?data=${encodedState}`, "_blank");
                      }}
                    >
                      <img
                        src={item.image_url}
...
export default ColourContactLens
;
